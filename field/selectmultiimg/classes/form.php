...
class dataformfield_selectmultiimg_form extends mod_dataform\pluginbase\dataformfieldform {
    ...
    protected function field_definition() {
        ...
        // Options.
        $label = get_string('options', 'dataformfield_selectmultiimg');
        $mform->addElement('textarea', 'param1', $label, 'wrap="virtual" rows="10" cols="50"');

        // Options separator.
        $label = get_string('optionsseparator', 'dataformfield_selectmultiimg');
        $mform->addElement('select', 'param3', $label, array_map('current', $field->separator_types));

        // Allow add option.
        $label = get_string('allowaddoption', 'dataformfield_selectmultiimg');
